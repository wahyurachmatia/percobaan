class EhpFormInputHandler extends elementorModules.frontend.handlers.Base {
    ...
    applyMasks() {
        ...
        this.elements.$form.find('input[data-mask]').each(function () {
            ...
            if($input.hasClass('mask_control_@ev-ccard')){
                $input.closest('.ehp-form__field-group').css({
                    'position':'relative'
                })
            }
...
jQuery(window).on('elementor/frontend/init', () => {
    const calHandler = ($element) => {
        elementorFrontend.elementsHandler.addHandler(EhpFormInputHandler, {
            $element,
        });
    };
  
    elementorFrontend.hooks.addAction('frontend/element_ready/ehp-form.default', calHandler);
}); 
